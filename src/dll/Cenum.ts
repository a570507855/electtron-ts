...
export enum nCmdShow {
  SW_FORCEMINIMIZE = 11,  //最小化一个窗口，即使拥有该窗口的线程没有响应。仅当最小化来自其他线程的窗口时，才应使用此标志。
  SW_HIDE = 0,            //隐藏该窗口并激活另一个窗口。
  SW_MAXIMIZE = 3,        //最大化指定的窗口。
  SW_MINIMIZE = 6,        //最小化指定的窗口并以Z顺序激活下一个顶级窗口。
  SW_RESTORE = 9,         //激活并显示窗口。如果窗口最小化或最大化，则系统会将其还原到其原始大小和位置。恢复最小化窗口时，应用程序应指定此标志。
  SW_SHOW = 5,            //激活窗口并以其当前大小和位置显示它。
  SW_SHOWDEFAULT = 10,    //根据启动应用程序的程序传递给CreateProcess函数的STARTUPINFO结构中指定的SW_值设置显示状态。
  SW_SHOWMAXIMIZED = 3,   //激活窗口并将其显示为最大化窗口。
  SW_SHOWMINIMIZED = 2,   //激活该窗口并将其显示为最小化窗口。
  SW_SHOWMINNOACTIVE = 7, //将窗口显示为最小化窗口。该值类似于SW_SHOWMINIMIZED，除了未激活窗口。
  SW_SHOWNA = 8,          //以当前大小和位置显示窗口。该值类似于SW_SHOW，除了窗口未激活。
  SW_SHOWNOACTIVATE = 4,  //以最近的大小和位置显示窗口。该值类似于SW_SHOWNORMAL，除了未激活窗口。
  SW_SHOWNORMAL = 1,      //激活并显示一个窗口。如果窗口最小化或最大化，则系统会将其还原到其原始大小和位置。首次显示窗口时，应用程序应指定此标志。
